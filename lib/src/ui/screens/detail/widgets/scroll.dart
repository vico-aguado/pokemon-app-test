...
class ScrollSnapListState extends State<ScrollSnapList> {
  ...
  void _animateScroll(double location) {
    Future.delayed(Duration.zero, () {
      if (widget.listController.hasClients) {
  ...
  double calculateScale(int index) {
    final intendedPixel = index * widget.itemSize;
    final difference = intendedPixel - currentPixel;

    if (widget.dynamicSizeEquation != null) {
      final scale = widget.dynamicSizeEquation!(difference);
  ...
  double calculateOpacity(int index) {
    final intendedPixel = index * widget.itemSize;
    final difference = intendedPixel - currentPixel;
    final nume = widget.itemSize;

    final double minimo = min(difference.abs() / nume, nume);

    final result = minimo > 1.0
        ? 1.0
        : minimo < 0.0
            ? 0.0
            : minimo;
  ...
  Widget _buildListItem(BuildContext context, int index) {
    ...
    if (widget.dynamicItemSize) {
      child = ColorFiltered(
        colorFilter: ColorFilter.mode(
          widget.fillColor.withOpacity(calculateOpacity(index)),
          BlendMode.srcATop,
        ),
    ...
    child = Opacity(
      opacity:
          1 - calculateOpacity(index) < 0.3 ? 0.3 : 1 - calculateOpacity(index),
      child: child,
    );
  ...
  double _calcCardLocation({
    ...
    var cardIndex = index ?? ((pixel! - itemSize / 2) / itemSize).ceil();
    ...
    return Container(
      ...
      child: LayoutBuilder(
        builder: (BuildContext ctx, BoxConstraints constraint) {
          final listPadding = calculateListPadding(constraint);
            ...
            child: NotificationListener<ScrollNotification>(
              onNotification: (ScrollNotification scrollInfo) {
                ...
                if (scrollInfo is ScrollEndNotification) {
                  ...
                  final offset = _calcCardLocation(
